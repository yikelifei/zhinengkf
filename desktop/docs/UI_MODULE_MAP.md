...
| 发送 | `/send/diagnostics` | 只读诊断适配器、发送任务、回执和身份校验证据 | 刷新诊断 |
| 发送 | `/send/diagnostics/operations` | 运行企业微信发送回执与发送异常扫描 | 扫描异常 |
...
| 设计 | `/design/zhenxi-ai` | 在客服桌面端内操作与独立桌面端同源的臻希 AI 工作区 | 刷新同步 |
...
| 设计 | `/design/jobs/new` | 从真实客户会话创建一条设计任务草稿 | 确认创建任务 |
...
| 设计 | `/design/jobs/[id]/quote` | 只由一条设计任务生成或返回报价草稿 | 确认生成报价 |
...
| 销售 | `/sales/quotes/[id]/verify-payment` | 记录一条报价的人工付款核验和内部支付台账 | 确认核验付款 |
...
| 销售 | `/sales/orders/[id]/edit` | 保存一条订单的字段并遵守履约状态机 | 确认保存字段 |
| 销售 | `/sales/orders/[id]/after-sales` | 创建并处理一条订单的售后 case，记录退款流水、补发物流或拒绝原因 | 记录售后处理结论 |
...
| 训练 | `/training/overview` | 汇总训练样本、质量、技能建议和 Agent 覆盖度 | 刷新训练概览 |
| 训练 | `/training/knowledge` | 检索和核对知识库条目、标签、来源和正文 | 刷新知识库 |
...
| 系统 | `/settings/ai-models` | 检查 AI 引擎、供应商、模型和配置问题 | 刷新模型状态 |
| 系统 | `/settings/delivery-readiness` | 汇总项目完成度审计、产品验收报告和外部阻塞项 | 刷新交付状态 |
...
- `/sales/overview` 转到 `/sales/quotes`，`/settings/accounts` 转到 `/integrations/wechat-work/settings`。
...
会话、发送任务、设计任务、商品、报价、订单、训练样本、Agent 和审核对象均使用路径级实体 ID；不存在的 ID 显示明确未找到状态，不再静默选中列表第一项。旧个人微信路径仅作为兼容入口保留，并重定向到企业微信接入页面。
