...
if not "%STABLE_SERVICE_WINDOW%"=="1" (
  ...
  if errorlevel 1 (
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0tools\start-stable-keepalive.ps1"
    if errorlevel 1 echo [%date% %time%] failed to start stable keepalive with %ERRORLEVEL% >> "%~dp0.runtime-stable\stable-service-window.log"
    exit /b 0
  )
  exit /b 0
)
...
set STABLE_SERVICE_LOG=%~dp0.runtime-stable\stable-service-window.log
...
if not exist "%~dp0apps\web\.next\standalone\apps\web\server.js" echo [%date% %time%] web standalone missing; runtime will use Next dev fallback >> "%STABLE_SERVICE_LOG%"
:enter_keepalive
