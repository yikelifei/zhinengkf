...
function main() {
  ...
  if (supervisorResult.status === 0) {
    ...
    console.log(`[launch] stable supervisor ${supervisorArgs.join(" ")} pid=${pid}`);
    return;
  }
...
function buildLauncherCmd() {
  const lines = [
    ...
    `echo [%date% %time%] desktop-service-supervisor exited with %ERRORLEVEL%, restarting >> ${cmdQuote(launcherLog)}`,
