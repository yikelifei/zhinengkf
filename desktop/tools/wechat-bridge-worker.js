...
async function runOnce(config = readConfig()) {
  ...
  await Promise.all(pending.map(async (entry) => {
    const accountId = String(entry.wechatAccountId || "unknown");
    const lock = acquireAccountLock(accountId, config, entry);
    if (!lock) {
...
function buildAckPayload(entry, mode, outboxPayload = {}) {
  ...
  const outboxTarget = outboxPayload && typeof outboxPayload.target === "object" && outboxPayload.target ? outboxPayload.target : {};
  return {
    version: BRIDGE_ACK_VERSION,
    ...
    wechatAccountId: String(entry.wechatAccountId || ""),
    conversationId: String(entry.conversationId || ""),
    customerId: String(entry.customerId || outboxPayload.customerId || outboxTarget.customerId || ""),
...
function acquireAccountLock(accountId, config, entry = {}) {
  fs.mkdirSync(config.lockDir, { recursive: true });
