...
test("developer startup scripts default to the current design mode", () => {
  ...
  assert.match(stableDoctor, /expected = \[webRuntimeServerPath, nextCliPath\]\.map\(normalizePathText\)/);
...
test("double click startup bat files use stable launcher scripts", () => {
  ...
  assert.match(stableKeepalive, /if %ERRORLEVEL% EQU 0 exit \/b 0/);
  ...
  assert.match(stableKeepalivePs1, /stable-runtime-launcher\.js/);
  assert.match(stableKeepalivePs1, /System\.Diagnostics\.ProcessStartInfo/);
  assert.match(stableKeepalivePs1, /CreateNoWindow = \$true/);
  assert.doesNotMatch(stableKeepalivePs1, /start `\"SmartKefu Stable Runtime`\" \/min cmd\.exe \/k/);
  ...
  assert.match(stableLaunch, /keep that service window open/);
...
test("stable runtime launcher owns one stable runtime and required services", () => {
  ...
  assert.match(stableRuntime, /fs\.existsSync\(webStandaloneServerPath\)/);
  ...
  assert.match(stableRuntime, /web standalone build missing; using Next dev server fallback/);
  ...
  assert.match(stableRuntime, /setInterval\(\(\) => \{\s+killStaleRuntimeProcesses\(\);\s+for \(const spec of specs\) ensureService\(spec\);\s+\}, 5000\);/);
  ...
  assert.match(stableRuntime, /name = 'node\.exe' OR name = 'cmd\.exe'/);
  assert.match(stableRuntime, /zhinengkefu_restore_work\|runtime-d-repo/);
  assert.match(stableRuntime, /supervisor-child\|stable-supervise\|supervise\|launch\|web-standalone-server/);
  assert.match(stableRuntime, /start-dev-ports\|desktop-service-supervisor/);
  assert.match(stableRuntime, /\[.\]runtime/);
  assert.match(stableRuntime, /--keep-alive\|--supervisor-child/);
  assert.match(stableRuntime, /supervisor-child-mock\[.\]cmd/);
  assert.match(stableRuntime, /killing stale runtime process pid\(s\)/);
});
...
test("port stack launcher blocks mock when real mode is active and starts supervised services", () => {
  ...
  assert.match(launcher, /function apiHealthUsesRuntimeDir\(apiHealth\)/);
