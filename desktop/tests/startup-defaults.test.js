...
test("developer startup scripts default to the current design mode", () => {
  ...
  assert.match(stableDoctor, /const allowDesignUnavailable = args\.has\("--allow-design-unavailable"\)/);
  assert.match(stableDoctor, /warnWhenFailed: allowDesignUnavailable/);
  assert.match(stableDoctor, /ZHENXI_BROWSER_EMBED_PORT", 3710/);
  assert.match(stableDoctor, /Zhenxi browser embed/);
...
test("stop script recognizes child service command lines", () => {
  ...
  assert.match(stopDevPorts, /ZHENXI_BROWSER_EMBED_PORT", 3710/);
...
test("double click startup bat files use stable launcher scripts", () => {
  ...
  assert.match(stableKeepalivePs1, /Invoke-CimMethod -ClassName Win32_Process -MethodName Create/);
  assert.match(stableKeepalivePs1, /Durable = \$true/);
  assert.match(stableKeepalivePs1, /durable supervisor handoff unavailable; using hidden process fallback/);
  ...
  assert.match(stableAppLauncher, /desktopWindowHeartbeatFresh/);
  assert.match(stableAppLauncher, /readyWindowAllowsRecovery/);
  ...
  assert.match(stableAppLauncher, /"--allow-design-unavailable"/);
  assert.match(stableAppLauncher, /launch-stable-electron-visible\.ps1/);
  assert.match(stableAppLauncher, /"launch Electron", \{ captureOutput: false \}/);
  assert.match(stableAppLauncher, /captureOutput \? \{ encoding: "utf8" \} : \{ stdio: "ignore" \}/);
  ...
  const electronVisibleLauncher = readText("tools/launch-stable-electron-visible.ps1");
  assert.match(electronVisibleLauncher, /electron-\$InstanceId\.status\.json/);
  assert.match(electronVisibleLauncher, /electron-\$InstanceId\.request\.json/);
  assert.match(electronVisibleLauncher, /Get-ReadyInstanceStatus/);
  assert.match(electronVisibleLauncher, /\[DateTimeOffset\]::Parse/);
  assert.match(electronVisibleLauncher, /\$Status\.updatedAt -is \[DateTime\]/);
  assert.doesNotMatch(electronVisibleLauncher, /Wait-Process -Id \$VisiblePid/);
...
test("stable startup entrypoints resolve the active worktree and stay fail-closed", () => {
  ...
  assert.match(stableLaunch, /start-hidden-stable-app\.ps1" -Mode open/);
  assert.match(electronMain, /let webUrl = normalizeSmartKefuWebUrl\(process\.env\.WEB_URL\)[\s\S]*requestedWebUrlFromCommandLine\(process\.argv\)[\s\S]*DEFAULT_WEB_URL/);
  assert.match(electronMain, /function normalizeSmartKefuWebUrl\(value\)/);
  assert.match(electronMain, /Number\(parsed\.port \|\| 80\) !== 3100/);
  assert.match(electronMain, /requestedWebUrlFromCommandLine\(commandLine\)/);
  assert.match(electronMain, /if \(!mainWindow \|\| mainWindow\.isDestroyed\(\)\) \{\s*createMainWindow\(\)/);
  assert.match(electronMain, /destroyZhenxiEmbeddedView\(\);\s*loadMainWindowUrl\(\)/);
  assert.match(electronMain, /mainWindow\.setSkipTaskbar\(false\)/);
  assert.match(electronMain, /mainWindow\.moveTop\(\)/);
  assert.match(electronMain, /render-process-gone/);
  assert.match(electronMain, /webUrl = endpoints\.webOverviewUrl/);
  assert.match(electronMain, /app\.disableHardwareAcceleration\(\)/);
  assert.match(electronMain, /app\.commandLine\.appendSwitch\("disable-gpu"\)/);
  assert.match(electronMain, /app\.commandLine\.appendSwitch\("disable-gpu-compositing"\)/);
  assert.match(electronMain, /app\.commandLine\.appendSwitch\("in-process-gpu"\)/);
  assert.match(electronMain, /app\.commandLine\.appendSwitch\("disable-crash-reporter"\)/);
  assert.match(stableKeepalivePs1, /Test-StableHttp "http:\/\/127\.0\.0\.1:3100\/overview"/);
...
test("stable runtime launcher owns one stable runtime and required services", () => {
  ...
  assert.match(stableRuntime, /name: "zhenxi-browser-embed"/);
  assert.match(stableRuntime, /zhenxi-browser-embed-proxy\.js/);
  assert.match(stableRuntime, /__smart_kefu_embed_health/);
  ...
  assert.match(stableRuntime, /const portOwnerStartupGraceMs = positiveNumber\(process\.env\.STABLE_PORT_OWNER_STARTUP_GRACE_MS, 15000\)/);
  ...
  assert.match(stableRuntime, /USE_LOCAL_STORE: "false"/);
  ...
  assert.match(stableRuntime, /portOwnerWithinStartupGrace\(spec, pid\)/);
  assert.match(stableRuntime, /inside startup grace; waiting for health check before isolation/);
  ...
  assert.match(stableRuntime, /owner\(s\) \$\{owners\.join\(","\)\} are starting; deferring duplicate launch/);
  assert.match(stableRuntime, /function rememberPortOwners\(spec, owners\)/);
  assert.match(stableRuntime, /function portOwnerWithinStartupGrace\(spec, pid\)/);
  ...
  assert.match(stableRuntime, /const existingHasFreshHeartbeat = existingPid > 0 && freshHeartbeatMatchesPid\(existingPid\)/);
  assert.match(stableRuntime, /const existingIsCurrent = existingIsAlive && existingHasFreshHeartbeat/);
  assert.match(stableRuntime, /const existingIsInitializing = existingIsAlive && lockAgeMs < 5000/);
  assert.doesNotMatch(stableRuntime, /if \(existingIsAlive \|\| existingHasFreshHeartbeat/);
  assert.match(stableRuntime, /function freshHeartbeatMatchesPid\(pid\)/);
  assert.match(stableRuntime, /process\.on\("uncaughtException", \(error\) => \{[\s\S]*process\.exit\(1\)/);
  ...
  assert.match(stableDoctor, /directServicePids\.length >= requiredServicePorts\.length/);
...
test("electron startup failure points beginners to stable repair script", () => {
  ...
  assert.match(electronMain, /function showMainWindow\(\)/);
  assert.match(electronMain, /show: true/);
  assert.match(electronMain, /mainWindow\.webContents\.on\("did-finish-load", showMainWindow\)/);
  assert.match(electronMain, /setTimeout\(showMainWindow, 3000\)\.unref\(\)/);
  assert.match(electronMain, /if \(!mainWindow\.isVisible\(\)\) mainWindow\.show\(\)/);
  ...
  assert.match(stableElectron, /const profileId = `\$\{instanceId\}-v2`/);
  assert.match(stableElectron, /electron-user-data", profileId/);
  assert.doesNotMatch(stableElectron, /if \(instanceId !== "default"\)/);
  assert.match(stableElectron, /electronArgs\.push\(`--user-data-dir=\$\{userDataDir\}`\)/);
  assert.match(stableElectron, /"--no-sandbox"/);
  assert.match(stableElectron, /"--disable-gpu"/);
  assert.match(stableElectron, /"--disable-gpu-compositing"/);
  assert.match(stableElectron, /"--disable-gpu-sandbox"/);
  assert.match(stableElectron, /"--disable-accelerated-2d-canvas"/);
  assert.match(stableElectron, /"--disable-accelerated-video-decode"/);
  assert.match(stableElectron, /"--disable-zero-copy"/);
  assert.match(stableElectron, /"--in-process-gpu"/);
  assert.doesNotMatch(stableElectron, /"--single-process"/);
  assert.match(stableElectron, /"--disable-crash-reporter"/);
  assert.match(stableElectron, /electron-launch\.err\.log/);
  assert.match(stableElectron, /code === 0 && !signal/);
  assert.match(stableElectron, /Electron handed off to an existing single-instance window/);
  assert.match(stableElectron, /Electron exited before the window stayed open/);
  assert.match(stableElectron, /const requestedWebUrl = process\.env\.WEB_URL \|\| "http:\/\/127\.0\.0\.1:3100\/overview"/);
  assert.match(stableElectron, /--smart-kefu-web-url=\$\{requestedWebUrl\}/);
  assert.match(stableElectron, /spawn\(electronPath, electronArgs/);
  assert.match(stableElectron, /env\.DESKTOP_INSTANCE_ID = instanceId/);
  assert.match(stableElectron, /env\.DESKTOP_RUNTIME_DIR = runtimeDir/);
  assert.match(stableElectron, /detached: false/);
  assert.match(stableElectron, /windowsHide: false/);
  assert.doesNotMatch(stableElectron, /child\.unref\(\)/);
  const appModule = readText("apps/api/src/app.module.ts");
  const desktopShell = readText("apps/api/src/desktop-shell/desktop-shell.controller.ts");
  assert.match(appModule, /DesktopShellController/);
  assert.match(desktopShell, /@Controller\("desktop-shell"\)/);
  assert.match(desktopShell, /@Post\("open-zhenxi-ai"\)/);
  assert.match(desktopShell, /@RequireOperatorCapability\("view_console"\)/);
  assert.match(desktopShell, /desktop_shell_interactive_launch_required/);
  assert.match(desktopShell, /打开臻希AI\.cmd/);
  assert.doesNotMatch(desktopShell, /child_process|spawn|taskkill/i);
  const visibleElectron = readText("tools/launch-stable-electron-visible.ps1");
  assert.match(visibleElectron, /Start-Process -FilePath \$Electron/);
  assert.match(visibleElectron, /-WindowStyle Normal/);
  assert.match(visibleElectron, /Get-ReadyInstanceStatus/);
  assert.match(visibleElectron, /\$CurrentUrl\.AbsoluteUri -ne \$TargetUri\.AbsoluteUri/);
  assert.match(visibleElectron, /handledRequestId/);
  assert.match(visibleElectron, /if \(-not \$RunningStatus\)/);
  assert.match(visibleElectron, /\$Owner\.Path -ne \$Electron/);
  assert.match(visibleElectron, /did not confirm a visible desktop window at the requested route/);
  assert.match(visibleElectron, /Write-LaunchStatus -Ok \$true/);
  assert.doesNotMatch(visibleElectron, /Wait-Process -Id \$VisiblePid/);
  assert.match(visibleElectron, /--smart-kefu-web-url=/);
  assert.doesNotMatch(visibleElectron, /DESKTOP_WEB_SESSION_PROOF|desktop-web-session\.json/);
  assert.doesNotMatch(desktopShell, /body|query|params/i);
