...
test("double click startup bat files use stable launcher scripts", () => {
  ...
  assert.match(stableStart, /ports:stop >> "%STABLE_SERVICE_LOG%" 2>>&1/);
  ...
  assert.match(stableStart, /stable:doctor -- --wait --wait-ms=15000 --interval-ms=3000/);
  ...
  assert.match(stableKeepalivePs1, /\$TaskName = "zhinengkefu_stable_runtime"/);
  assert.match(stableKeepalivePs1, /System32\\schtasks\.exe/);
  assert.match(stableKeepalivePs1, /stable services already healthy; watchdog start skipped/);
  assert.match(stableKeepalivePs1, /\$Schtasks \/Create \/TN \$TaskName \/SC MINUTE \/MO 1/);
  assert.match(stableKeepalivePs1, /\$Schtasks \/Run \/TN \$TaskName/);
  ...
  assert.match(stableKeepalivePs1, /CurrentDirectory = \$Root/);
  assert.match(stableKeepalivePs1, /start `"\$WindowTitle`" \/min cmd\.exe \/d \/k/);
  assert.match(stableKeepalivePs1, /Get-CimInstance Win32_Process -Filter "name = 'cmd\.exe'"/);
  assert.match(stableKeepalivePs1, /Start-Process -FilePath "cmd\.exe"/);
  assert.match(stableKeepalivePs1, /"\/k"/);
  assert.match(stableKeepalivePs1, /"-NoExit"/);
