...
test("panel renders four roles and nine capabilities as a semantic matrix", () => {
  const html = renderPanel();
  assert.match(html, /<table>/);
  assert.match(html, /<caption[^>]*>管理员、主管、客服和只读角色的九项能力矩阵<\/caption>/);
  for (const label of ["管理员", "主管", "客服", "只读"]) assert.match(html, new RegExp(label));
  for (const label of ["查看工作台", "管理渠道", "管理会话分配", "回复客户会话", "审批发送", "管理设计执行", "管理订单履约", "管理训练", "管理角色"]) {
