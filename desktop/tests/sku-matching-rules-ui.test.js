...
test("sku image problems can be searched and exported as the visible repair list", () => {
  ...
  assert.match(page, /\.map\(\(imagePath, index\) => \(\{ imagePath, index \}\)\)/);
  assert.match(page, /多角度图原第 \$\{index \+ 1\} 张：\$\{imagePath\}/);
  assert.match(page, /多角度图新第 \$\{index \+ 1\} 张：\$\{imagePath\}/);
  assert.match(page, /SKU 图片待保存变更：\$\{form\.skuCode\.trim\(\)\}｜\$\{form\.name\.trim\(\) \|\| originalSku\.name \|\| "未命名商品"\}/);
  assert.match(page, /复核口径：只核对图片路径变更，不改价格、库存、搭配规则和商品上下架状态。/);
  ...
  assert.match(page, /skuImageProblemTrackingId\(problem\),/);
  ...
  assert.match(page, /\["SKU编号", "商品名称", "问题标识", "同商品图片问题数"/);
  assert.match(page, /skuImageProblemTrackingId\(problem\),/);
  ...
  assert.match(page, /问题标识：\$\{skuImageProblemTrackingId\(item\)\}/);
  assert.match(page, /当前筛选：\$\{visibleSkuImageProblemFilterSummary\}。/);
  assert.match(page, /审核刷新口径：\$\{skuImageProblemAuditRefreshContext\}。/);
  ...
  assert.match(page, /问题标识：\$\{skuImageProblemTrackingId\(problem\)\}/);
  assert.match(page, /商品：\$\{problem\.skuCode \|\| "未编号"\}｜\$\{problem\.name \|\| "未命名商品"\}｜\$\{skuImageRoleLabel\(problem\)\}/);
  assert.match(page, /路径：\$\{problem\.path\}/);
  assert.match(page, /路径核对信息/);
  ...
  assert.match(page, /function skuImageProblemTrackingId\(/);
  assert.match(page, /return `\$\{productKey\}::\$\{problem\.field\}::\$\{imagePosition\}::\$\{problem\.code\}`/);
  assert.match(page, /问题标识：\$\{skuImageProblemTrackingId\(problem\)\}/);
  assert.match(page, /当前筛选：\$\{visibleSkuImageProblemFilterSummary\}。/);
  assert.match(page, /审核刷新口径：\$\{skuImageProblemAuditRefreshContext\}。/);
  assert.match(page, /回填格式：结果=可用\/需重传\/已移除；新路径=本地真实图片路径；备注=处理说明。/);
  assert.match(page, /回填：结果=；新路径=；备注=/);
  ...
  assert.match(page, /回填格式：新路径=本地真实图片路径；来源=拍摄\/供应商\/设计平台；备注=处理说明。/);
  assert.match(page, /回填：新路径=；来源=；备注=/);
  ...
  assert.match(page, /回填格式：结果=文件存在\/已重传\/已移除并补新图；新路径=本地真实图片路径；备注=处理说明。/);
  ...
  assert.match(page, /分组涉及商品：\$\{visibleSkuImageProblemActionProductSummary\}。/);
  assert.match(page, /下一步：\$\{visibleSkuImageProblemNextStepSummary\}。/);
  assert.match(page, /问题标识列表：\$\{visibleSkuImageProblems\.map\(\(problem\) => skuImageProblemTrackingId\(problem\)\)\.join\("、"\)\}。/);
