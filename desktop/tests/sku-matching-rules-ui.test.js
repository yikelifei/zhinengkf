...
test("sku image problems can be searched and exported as the visible repair list", () => {
  ...
  assert.match(page, /function buildSkuFormImageChangeCopyText\(form: SkuForm, originalSku: Sku \| null \| undefined\)/);
  assert.match(page, /SKU 图片待保存变更：\$\{form\.skuCode\.trim\(\)\}｜\$\{form\.name\.trim\(\) \|\| originalSku\.name \|\| "未命名商品"\}/);
  assert.match(page, /下一步：确认路径无误后点击“保存商品”，再点击“刷新商品审核”确认图片问题减少。/);
  ...
  assert.match(page, /if \(!problem\.path\) \{/);
  assert.match(page, /setMessage\(`[^`]*\$\{skuImageRoleLabel\(problem\)\}[^`]*旧路径[^`]*编辑图片[^`]*`\)/);
  assert.match(page, /const targetImageIndex = problem\.imageIndex === null \|\| problem\.imageIndex === undefined \? -1 : Number\(problem\.imageIndex\)/);
  assert.match(page, /const targetPath =/);
  assert.match(page, /targetImageIndex >= 0/);
  ...
  assert.match(page, /const targetImageIndex = problem\.imageIndex === null \|\| problem\.imageIndex === undefined \? -1 : Number\(problem\.imageIndex\)/);
  assert.match(page, /if \(targetImageIndex >= 0\) return imageIndex !== targetImageIndex/);
  assert.doesNotMatch(page, /imageIndex !== imageIndex/);
  assert.match(page, /setSkuIssueFilter\("missing_image"\)/);
  assert.match(page, /const confirmed = window\.confirm\(/);
  assert.match(page, /原路径：\$\{targetPath \|\| "未填写"\}/);
  ...
  assert.match(page, /async function copySkuFormImageChangeSummary\(\)/);
  assert.match(page, /buildSkuFormImageChangeCopyText\(skuForm, activeSkuOriginal\)/);
  assert.match(page, /await navigator\.clipboard\.writeText\(text\)/);
  assert.ok(page.includes("已复制 ${skuForm.skuCode || \"当前商品\"} 的待保存图片变更。"));
  assert.match(page, /onClick=\{copySkuFormImageChangeSummary\}/);
  assert.ok(page.includes("复制变更"));
  ...
  assert.match(page, /setMessage\(`[^`]*\$\{sku\.skuCode\}[^`]*\$\{skuImageRoleLabel\(problem\)\}[^`]*\$\{targetPath \|\| "未填写"\}[^`]*`\)/);
