...
function setup(aiProviders) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "wechat-ai-model-reply-"));
...
test("safe automatic inbound reply uses the configured model after rules and scoped knowledge", async () => {
  const calls = [];
  const ai = {
    generateInboundSuggestion: async (input) => {
      calls.push(input);
      return { text: "收到，我帮您核对订单 123 的物流进度。", provider: "zhipu", model: "glm-test", attempts: 2 };
    },
  };
  ...
  const result = await service.processInboundMessage({
    ...
    text: "物流快递单号 123 已停滞三天，请帮我查物流进度",
  });
...
test("manual-risk inbound never calls a model and still fails closed to human review", async () => {
  let calls = 0;
...
test("provider failure keeps the existing rule reply and does not interrupt inbound processing", async () => {
