...
3. 精确身份哈希匹配：候选图保存时会生成 `fingerprint`；只有上游提供同一稳定身份值时才能精确匹配候选图。
4. 裁剪、压缩或截图相似匹配尚未实现感知哈希，不能精确匹配时系统不会硬猜，会创建提醒并进入人工确认。

当前 `fingerprint` 是由任务 ID、请求 ID、候选图 ID、位置和下载 URL 等元数据生成的稳定 SHA-256 身份哈希，不是图片字节哈希或感知哈希。正式部署可优先使用设计平台返回的稳定图片 ID/文件哈希；需要截图相似匹配时必须另行实现并验证感知哈希。
...
- 客户选图识别：支持文字编号、候选图 ID/引用 ID 和稳定 SHA-256 身份哈希精确匹配；不确定时转人工确认。该哈希基于任务/图片元数据，不是图片字节哈希或感知哈希。
...
- 当前图片指纹只是基于任务/图片元数据的稳定 SHA-256 身份哈希；裁剪、压缩或截图的感知相似匹配尚未实现，不得宣称已经支持感知哈希。
