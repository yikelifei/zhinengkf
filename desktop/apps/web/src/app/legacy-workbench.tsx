...
// Legacy migration archive. Modular routes do not import or render this component.
...
import {
  ...
  type PersonalWechatWorkspaceView,
...
import {
  getWorkbenchRouteForSection,
  getWorkbenchRouteSelection,
  type AccountWorkbenchView,
  type AutomationWorkbenchView,
  type CatalogWorkbenchView,
  type PersonalWechatRouteView,
  type ReviewWorkbenchView,
  type SalesWorkbenchView,
  type SendWorkbenchView,
  type SkuWorkbenchView,
  type TrainingWorkbenchView,
  type WechatWorkbenchView,
  type WorkbenchRoute,
...
export type LegacyWorkbenchProps = {
  route: WorkbenchRoute;
};

export default function LegacyWorkbench({ route }: LegacyWorkbenchProps) {
  const router = useRouter();
  const initialRouteSelection = getWorkbenchRouteSelection(route);
  ...
  const [trainingWorkbenchView, setTrainingWorkbenchViewState] = useState<TrainingWorkbenchView>(
    initialRouteSelection.training || "import",
  );
  const [wechatWorkbenchView, setWechatWorkbenchViewState] = useState<WechatWorkbenchView>(
    initialRouteSelection.wechat || "channels",
  );
  const [personalWechatWorkspaceView, setPersonalWechatWorkspaceView] = useState<PersonalWechatWorkspaceView>(
    initialRouteSelection.personalWechat === "safety"
      ? "safety"
      : initialRouteSelection.personalWechat === "control"
        ? "voice"
        : "accounts",
  );
  const [accountWorkbenchView, setAccountWorkbenchViewState] = useState<AccountWorkbenchView>(
    initialRouteSelection.account || "wechat",
  );
  const [sendWorkbenchView, setSendWorkbenchViewState] = useState<SendWorkbenchView>(
    initialRouteSelection.send || "queue",
  );
  const [reviewWorkbenchView, setReviewWorkbenchViewState] = useState<ReviewWorkbenchView>(
    initialRouteSelection.review || "handoff",
  );
  ...
  const [quoteWorkbenchView, setQuoteWorkbenchViewState] = useState<SalesWorkbenchView>(
    initialRouteSelection.sales || "overview",
  );
...
客服：先查物流，停滞就催件或补发。`);
  ...
  const [skuWorkbenchView, setSkuWorkbenchViewState] = useState<SkuWorkbenchView>(
    initialRouteSelection.sku || "catalog",
  );
  const [catalogWorkbenchView, setCatalogWorkbenchViewState] = useState<CatalogWorkbenchView>(
    initialRouteSelection.catalog || "import",
  );
  ...
  const [activeWorkspaceSection, setActiveWorkspaceSection] = useState<WorkspaceSectionId>(route.sectionId);
  const [noticeWorkbenchView, setNoticeWorkbenchViewState] = useState<AutomationWorkbenchView>(
    initialRouteSelection.automation || "automation",
  );
  const routeSelectionRef = useRef<WorkbenchRouteSelection>(initialRouteSelection);
...
CARD-B\t感谢卡B\t配件\t贺卡\t3\t12\t200\t客户拜访\tC:\\products\\card-b-main.jpg\t\t10*15\t20\t纸张\t本地印刷厂\t2\t\t{"mustWith":["BOX-B"]}`);
  ...
  useEffect(() => {
    const selection = getWorkbenchRouteSelection(route);
    routeSelectionRef.current = selection;
    setActiveWorkspaceSection(route.sectionId);
    setTrainingWorkbenchViewState(selection.training || "import");
    setWechatWorkbenchViewState(selection.wechat || "channels");
    setPersonalWechatWorkspaceView(
      selection.personalWechat === "safety" ? "safety" : selection.personalWechat === "control" ? "voice" : "accounts",
    );
    setAccountWorkbenchViewState(selection.account || "wechat");
    setSendWorkbenchViewState(selection.send || "queue");
    setReviewWorkbenchViewState(selection.review || "handoff");
    setQuoteWorkbenchViewState(selection.sales || "overview");
    setSkuWorkbenchViewState(selection.sku || "catalog");
    setCatalogWorkbenchViewState(selection.catalog || "import");
    setNoticeWorkbenchViewState(selection.automation || "automation");
    window.requestAnimationFrame(() => {
      document.querySelector<HTMLElement>(".workspace")?.scrollTo({ top: 0, left: 0, behavior: "auto" });
    });
  }, [route]);

  function updateSubviewRoute(
    sectionId: WorkspaceSectionId,
    selection: WorkbenchRouteSelection,
  ) {
    const nextSelection = { ...routeSelectionRef.current, ...selection };
    routeSelectionRef.current = nextSelection;
    if (activeWorkspaceSection === sectionId) {
      router.push(getWorkbenchRouteForSection(sectionId, nextSelection).href);
    }
  }

  function setTrainingWorkbenchView(view: TrainingWorkbenchView) {
    setTrainingWorkbenchViewState(view);
    updateSubviewRoute("training-center", { training: view });
  }

  function setWechatWorkbenchView(view: WechatWorkbenchView) {
    setWechatWorkbenchViewState(view);
    updateSubviewRoute("wechat-channel-center", { wechat: view });
  }

  function setPersonalWechatRouteView(view: PersonalWechatRouteView) {
    updateSubviewRoute("personal-wechat-center", { personalWechat: view });
  }

  function changePersonalWechatWorkspaceView(view: PersonalWechatWorkspaceView) {
    setPersonalWechatWorkspaceView(view);
    setPersonalWechatRouteView(view === "safety" ? "safety" : view === "voice" ? "control" : "instances");
  }

  function setAccountWorkbenchView(view: AccountWorkbenchView) {
    setAccountWorkbenchViewState(view);
    updateSubviewRoute("account-center", { account: view });
  }

  function setSendWorkbenchView(view: SendWorkbenchView) {
    setSendWorkbenchViewState(view);
    updateSubviewRoute("send-center", { send: view });
  }

  function setReviewWorkbenchView(view: ReviewWorkbenchView) {
    setReviewWorkbenchViewState(view);
    updateSubviewRoute("review-center", { review: view });
  }

  function setQuoteWorkbenchView(view: SalesWorkbenchView) {
    setQuoteWorkbenchViewState(view);
    updateSubviewRoute("quote-center", { sales: view });
  }

  function setSkuWorkbenchView(view: SkuWorkbenchView) {
    setSkuWorkbenchViewState(view);
    updateSubviewRoute("sku-library", { sku: view });
  }

  function setCatalogWorkbenchView(view: CatalogWorkbenchView) {
    setCatalogWorkbenchViewState(view);
    updateSubviewRoute("catalog-center", { catalog: view });
  }

  function setNoticeWorkbenchView(view: AutomationWorkbenchView) {
    setNoticeWorkbenchViewState(view);
    updateSubviewRoute("notice-center", { automation: view });
  }
  ...
  async function focusConversation(conversationId: string, sectionId: WorkspaceSectionId) {
  ...
  function setCatalogWorkbenchDetail(view: CatalogWorkbenchView) {
    setCatalogWorkbenchView(view);
  }

  function scrollToWorkspaceSection(
    sectionId: WorkspaceSectionId,
    options: { behavior?: ScrollBehavior; replace?: boolean } = {},
  ) {
    const behavior = options.behavior ?? "smooth";
    setActiveWorkspaceSection(sectionId);
    const targetRoute = getWorkbenchRouteForSection(sectionId, routeSelectionRef.current);
    if (targetRoute.href !== route.href) {
      if (options.replace) router.replace(targetRoute.href);
      else router.push(targetRoute.href);
    }
    ...
    >
      ...
      >
        ...
        <OperationsOverview
          ...
          busy={Boolean(busy)}
        />
        {activeWorkspaceSection === "personal-wechat-center" ? <PersonalWechatWorkspace
          ...
          onActiveViewChange={changePersonalWechatWorkspaceView}
          onRefresh={() => void load()}
