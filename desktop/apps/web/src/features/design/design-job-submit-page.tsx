...
export function DesignJobSubmitPage({ jobId, initialIdentityFilters = {} }: { jobId: string; initialIdentityFilters?: IdentityFilters }) {
      ...
      {loading && !loaded ? <DesignEmpty title="正在读取设计任务" detail={`任务 ${jobId}`} busy /> : selected ? (
        <article className={styles.card}>
          ...
          <div className={styles.actionRow}>
            <button type="button" data-action-id="design-job-submit-preflight" disabled={Boolean(busy) || Boolean(loadError) || !identityReady} onClick={() => void inspect()}><SearchCheck size={16} aria-hidden="true" />{busy === "preflight" ? "预检中" : "执行预检"}</button>
            <button type="button" className={styles.primaryButton} data-action-id="design-job-submit-request" disabled={Boolean(busy) || Boolean(loadError) || !preflight?.ok || preflight.designJobId !== selected.id} onClick={() => setConfirming(true)}><Play size={16} aria-hidden="true" />准备正式提交</button>
          </div>
          {preflight ? <div className={styles.preflight}><strong>预检 {preflight.ok ? "通过" : "未通过"}</strong><ul>{preflight.checks.map((check) => <li className={check.ok ? styles.ok : styles.bad} key={check.key}><span>{check.label}</span><small>{check.detail || "无明细"}</small>{check.action ? <small>{check.action}</small> : null}</li>)}</ul></div> : null}
          <Link className={styles.backLink} href={designIdentityHref(`/design/jobs/${encodeURIComponent(selected.id)}`, selected)} data-action-id="design-job-submit-back">返回任务详情</Link>
        </article>
