        ...
        <div className={styles.twoColumn}>
          ...
          <div className={styles.stack}>
            <article className={styles.card}><div className={styles.cardHeader}><div><h2>实时状态</h2><p>来自独立健康与就绪接口。</p></div></div>
              <dl className={styles.factGrid}><div><dt>健康</dt><dd>{health ? (health.ok ? "正常" : "异常") : "不可用"}</dd></div><div><dt>延迟</dt><dd>{health ? `${health.latencyMs} ms` : "—"}</dd></div><div><dt>正式提交</dt><dd>{readiness?.canSubmitFormalGeneration ? "允许" : "不允许"}</dd></div><div><dt>适配器</dt><dd>{readiness?.adapter || config?.config.adapter || "—"}</dd></div></dl>
