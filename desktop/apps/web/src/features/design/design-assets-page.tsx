...
export function DesignAssetsPage() {
  ...
  function invalidateAssetOperations(identity: DesignAssetReadIdentity) {
    assetOperationGuard.setIdentity(identity);
    setAssets([]);
    ...
    <section className={styles.page} aria-label="设计素材管理">
      ...
      <div className={styles.twoColumn}>
        <form className={styles.card} onSubmit={(event) => { event.preventDefault(); if (!identityReady) { setError("上传前必须补齐微信账号、会话和客户三项身份。"); return; } if (!file) { setError("请先选择要上传的文件。"); return; } setPendingConfirmation(true); }}>
          ...
          <div className={styles.formGrid}><label><span>客户归属 ID</span><input value={ownerId} onChange={(event) => { const next = event.target.value; invalidateAssetOperations(assetReadIdentity({ ownerId: next })); setOwnerId(next); }} /></label><label><span>微信账号 ID</span><input value={wechatAccountId} onChange={(event) => { const next = event.target.value; invalidateAssetOperations(assetReadIdentity({ wechatAccountId: next })); setWechatAccountId(next); }} /></label><label><span>会话 ID</span><input value={conversationId} onChange={(event) => { const next = event.target.value; invalidateAssetOperations(assetReadIdentity({ conversationId: next })); setConversationId(next); }} /></label><label><span>客户 ID</span><input value={customerId} onChange={(event) => { const next = event.target.value; invalidateAssetOperations(assetReadIdentity({ customerId: next })); setCustomerId(next); }} /></label><label><span>素材角色</span><select value={role} onChange={(event) => setRole(event.target.value)}><option value="logo">客户 Logo</option><option value="reference">参考图</option><option value="attachment">附件</option></select></label><label><span>选择文件</span><input type="file" accept="image/*,.pdf" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label></div>
