...
export function DesignJobDetailPage({ jobId }: { jobId: string }) {
  const { selected, loading, error, refresh } = useDesignJobs(jobId);
          ...
          {selected.images?.length ? (
            <div className={styles.imageGallery} aria-label="设计候选图">
              {selected.images.map((image, index) => {
                ...
                return <figure className={styles.imageTile} key={image.id || image.imageId || index}>{src ? <img src={src} alt={`设计候选图 ${index + 1}`} /> : <div className={styles.imageUnavailable}>本地预览不可用</div>}<figcaption>{image.imageId || image.id || `候选图 ${index + 1}`}</figcaption></figure>;
              })}
            </div>
