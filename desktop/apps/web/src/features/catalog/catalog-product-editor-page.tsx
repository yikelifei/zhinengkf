...
export function CatalogProductEditorPage({ skuCode = "" }: { skuCode?: string }) {
  ...
  return (
    <section className={styles.page} aria-label={skuCode ? "编辑商品" : "新建商品"}>
      ...
      {!imageReady ? <CatalogNotice tone="warning">商品缺少主图或多角度图；设计任务只能套用带真实商品图的 SKU。</CatalogNotice> : null}
      {!loaded ? <CatalogEmpty title="商品状态未确认" detail="商品库尚未成功读取，已阻止新增或编辑保存。" /> : stale ? <CatalogEmpty title="商品真值需要刷新" detail="当前只有上一次可信快照；刷新成功前已阻止保存。" /> : skuCode && !selected ? <CatalogEmpty title="没有找到商品" detail="读取成功；请返回商品列表重新选择。" /> : (
        <form className={styles.card} onSubmit={(event) => { event.preventDefault(); const validation = validate(); if (validation) { setError(validation); return; } setConfirming(true); }}>
          ...
          <div className={styles.formGrid}>
            <label className={styles.fullField}><span>上传商品主图</span><input type="file" accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" disabled={busy} data-action-id="catalog-product-editor-main-image-file" onChange={(event) => { const file = event.target.files?.[0] || null; const validation = validateImageFile(file); if (validation) { setError(validation); event.target.value = ""; return; } setError(""); setMainImageFile(file); }} />{mainImageFile ? <small>待上传：{mainImageFile.name}</small> : <small>支持 PNG、JPG、WEBP，单张不超过 10 MB。</small>}</label>
            <label className={styles.fullField}><span>上传多角度图</span><input type="file" multiple accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" disabled={busy} data-action-id="catalog-product-editor-angle-image-files" onChange={(event) => { const files = Array.from(event.target.files || []); const validation = files.map(validateImageFile).find(Boolean) || ""; if (validation) { setError(validation); event.target.value = ""; return; } setError(""); setAngleImageFiles(files); }} />{angleImageFiles.length ? <small>待上传 {angleImageFiles.length} 张：{angleImageFiles.map((file) => file.name).join("、")}</small> : <small>可一次选择多张真实商品细节图。</small>}</label>
          </div>
          ...
          <div className={styles.formGrid}>
            ...
            <label className={styles.fullField}><span>多角度图路径或 URL（每行一个）</span><textarea rows={3} value={(draft.angleImages || []).join("\n")} disabled={busy} onChange={(event) => update("angleImages", parseImageList(event.target.value))} /></label>
