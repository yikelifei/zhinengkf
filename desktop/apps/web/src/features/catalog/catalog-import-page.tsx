...
export function CatalogImportPage() {
  ...
  async function previewImport() {
    ...
    try { const result = mode === "text" ? await previewSkuImportText(text) : await previewSkuImportFile(file!.name, await fileBase64(file!)); setPreview(result); if (!result.ok) setError("预览存在阻断错误，不能写入商品目录。"); else setNotice(`预览完成：可导入 ${result.rows.length} 行，跳过 ${result.skippedCount} 行。`); }
    catch (cause) { setError(catalogError(cause, "导入预览失败")); }
    finally { setBusy(""); }
  }
  ...
  return <section className={styles.page} aria-label="商品导入">
    ...
    <div className={styles.twoColumn}><section className={styles.card} aria-label="导入来源"><div className={styles.segmented} role="group" aria-label="商品导入来源"><button type="button" className={mode === "text" ? styles.active : ""} data-action-id="catalog-import-mode-text" aria-label="使用文本导入" aria-pressed={mode === "text"} onClick={() => { setMode("text"); setPreview(null); }}>粘贴文本</button><button type="button" className={mode === "file" ? styles.active : ""} data-action-id="catalog-import-mode-file" aria-label="使用文件导入" aria-pressed={mode === "file"} onClick={() => { setMode("file"); setPreview(null); }}>上传文件</button></div>{mode === "text" ? <label className={styles.textField}><span>CSV / 表格文本</span><textarea rows={14} value={text} onChange={(event) => { setText(event.target.value); setPreview(null); }} placeholder="skuCode,name,type,salePrice,costPrice,stock" /></label> : <label className={styles.fileField}><span>CSV 或 XLSX 文件</span><input type="file" accept=".csv,.xlsx,.xls,text/csv" onChange={(event) => { setFile(event.target.files?.[0] || null); setPreview(null); }} /></label>}<div className={styles.formActions}><button type="button" data-action-id="catalog-import-preview" aria-label="预览商品导入" disabled={Boolean(busy)} onClick={() => void previewImport()}><FileSearch size={16} aria-hidden="true" />{busy === "preview" ? "预览中" : "只预览"}</button></div></section>
