  ...
  return (
    <section className={styles.page} aria-label="商品修复队列">
      ...
      <article className={styles.card} aria-label="待修复商品">
        <div className={styles.cardHeader}><div><h2>待修复</h2><p>{loaded ? `${audit?.repairQueueCount || 0} 项 · 阻断 ${audit?.blockingRepairCount || 0}` : "读取未确认"}</p></div></div>
        {loading ? <CatalogEmpty title="正在读取修复队列" detail="队列来自实时商品审计。" busy /> : loaded && audit?.repairQueue?.length ? (
