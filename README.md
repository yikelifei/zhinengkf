...
客服平台会先判断客户消息属于哪个场景，再交给对应 Agent。当前覆盖礼盒设计、下单支付、物流异常、售后安抚、尺码推荐、售前转化和未分类兜底。

- `scene`：最终判断出来的业务场景。
- `agentKey`：要接手处理的智能体。
- `matchedKeywords`：这次判断命中的关键词，方便排查为什么分到这个场景。
- `sceneScore`：最终场景分数，分数越高说明命中越明确。
- `sceneScores`：候选场景排名，用来观察是否存在相近场景。
- `sceneDecision`：场景是否清晰。`clear` 可以继续自动处理；`weak` 会先追问；`ambiguous` 说明多个场景接近，涉及售后或订单时转人工。
- `sceneClarification`：当场景弱或多场景接近时，生成客户能看懂的确认问题和候选选项，避免把订单、售后、物流、设计图混着回复。
- `action`：下一步处理方式，可能是智能体处理、先补信息或转人工。

下单、定金、尾款、改地址、发票、订单状态会进入“下单支付 Agent”。这个 Agent 只做订单和付款信息核对，不跨客户、跨账号、跨会话处理订单，避免把 A 客户的问题回复给 B 客户。
